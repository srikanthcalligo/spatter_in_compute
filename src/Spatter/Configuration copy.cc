...
//Host Code
  ...
  if (timed) {
    ...
    uint32_t buf_size = /*sizeof(Float16_b)*/ 2 * single_tile_size * n_tiles;
    uint32_t dest_buf_size = /*sizeof(Float16_b)*/ 2 * single_tile_size;
    uint32_t buf_page_size = /*sizeof(Float16_b)*/ 2 * single_tile_size;
    uint32_t dbuf_page_size = /*sizeof(Float16_b)*/ 2 * single_tile_size;
    ...
    std::shared_ptr<tt::tt_metal::Buffer> dram_buffer2 = CreateBuffer(buffer_config_a);
...
//#ifdef PRINT_DEBUG
    printf("Destination array size = %zu %zu\n", result.size(), wrap);
    printf("Final : \n");
