...
void kernel_main()
{
    ...
    uint32_t pscatter_coord_x = get_arg_val<uint32_t>(1);
    uint32_t pscatter_coord_y =  get_arg_val<uint32_t>(2);
    ...
    noc_async_read(pscatter_dram_noc_addr, l1_write_addr_in1, ublock_size_bytes_1);
    noc_async_read_barrier();
    noc_async_read(dense_dram_noc_addr, l1_write_addr_in2, ublock_size_bytes_2);
    noc_async_read_barrier();
