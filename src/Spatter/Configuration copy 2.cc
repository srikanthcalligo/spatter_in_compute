...
//Host Code
  ...
  if (timed) {
...
#endif //End of Riscv version

#if 1 //Compute Kernel version
#ifdef TT_SPATTER_ENABLE
    constexpr CoreCoord core = {0,0};
    constexpr uint32_t device_id = 0; 
    Device *device = CreateDevice(device_id);
    ...
    uint32_t dest_buf_size = sizeof(bfloat16) * single_tile_size;
    uint32_t buf_page_size = sizeof(bfloat16) * single_tile_size;
    uint32_t dbuf_page_size = sizeof(bfloat16) * single_tile_size;
    ...
    std::shared_ptr<tt::tt_metal::Buffer> dram_buffer = CreateBuffer(buffer_config_a);
    std::shared_ptr<tt::tt_metal::Buffer> dram_buffer1 = CreateBuffer(buffer_config_b);
    //std::shared_ptr<tt::tt_metal::Buffer> dram_buffer3 = CreateBuffer(buffer_config_b);
    std::shared_ptr<tt::tt_metal::Buffer> dram_buffer2 = CreateBuffer(buffer_config_b);

    auto src0_coord =  dram_buffer->noc_coordinates();
    auto src1_coord = dram_buffer1->noc_coordinates();
    //auto src2_coord = dram_buffer3->noc_coordinates();
    auto dst_coord = dram_buffer2->noc_coordinates();
    ...
    CircularBufferConfig cb1_src1_config = CircularBufferConfig(
        buf_src0,
        {{src1_cb_index, tt::DataFormat::Float16_b}}).set_page_size(src1_cb_index, dbuf_page_size);
    ...
    CircularBufferConfig cb1_src2_config = CircularBufferConfig(
        dest_buf_size,
        {{src2_cb_index, tt::DataFormat::UInt32}}).set_page_size(src2_cb_index, dbuf_page_size);
    ...
    */
    ...
    CircularBufferConfig cb_dst_config = CircularBufferConfig(
        buf_src0,
        {{dst_cb_index, tt::DataFormat::Float16_b}}).set_page_size(dst_cb_index, dbuf_page_size);
    ...
    //std::vector<uint32_t> a_data = create_random_vector_of_bfloat16(single_tile_size * n_tiles * 2, 10, rng());
    //std::vector<uint32_t> b_data = create_random_vector_of_bfloat16(single_tile_size * n_tiles * 2, 10, rng());
#if 1
    //Declare pattern and sparse arrays
    std::vector<uint32_t> sparse_vec(single_tile_size * n_tiles);//(sparse.size());
    std::vector<uint32_t> pattern_mat_val(single_tile_size);
    //float sparse_vec[single_tile_size * n_tiles];
    //float pattern_mat_val[single_tile_size];
#ifdef PRINT_DEBUG
    printf("\nInput : ");
#endif
    for(uint32_t i=0, j = 0; i < (single_tile_size * n_tiles) ; i=i+2, j++){
      ...
      //if(i < 8){
      //  printf("%lf %f %f\n", sparse[i], num_1_bfloat16.to_float(), num_2_bfloat16.to_float());
      //}
      //Printing last 8 elements to validate the final data
#ifdef PRINT_DEBUG
      if(i > ((1024*n_tiles) - 9))
    ...
    for(int i=0 , j=0; i < single_tile_size ; i=i+stride_len, j++){
      ...
      //if(i < 8){
      // printf("%f ", pattern_mat_val[i].to_float());
      //}
    }
#endif
    EnqueueWriteBuffer(cq, dram_buffer, sparse_vec, false);
    ...
    SetRuntimeArgs(program, void_data_kernel_noc0_read, core, {dram_buffer->address(), dram_buffer1->address(),src0_coord.x, src0_coord.y, src1_coord.x, src1_coord.y, n_tiles, dram_buffer2->address(), dst_coord.x, dst_coord.y, pattern_length, delta, wrap});
#else
    SetRuntimeArgs(program, void_data_kernel_noc0_read, core, {dram_buffer->address(), dram_buffer1->address(),src0_coord.x, src0_coord.y, src1_coord.x, src1_coord.y, n_tiles, pattern_length, delta, wrap});
    //dram_buffer3->address(),src2_coord.x, src2_coord.y
    //SetRuntimeArgs(program, void_data_kernel_noc0_read, core, {dram_buffer->address(),src0_coord.x, src0_coord.y});
    SetRuntimeArgs(program, eltwise_binary_kernel_id, core, {n_tiles});
    SetRuntimeArgs(program, void_data_kernel_noc1_write, core, {dram_buffer2->address(), dst_coord.x, dst_coord.y, 1});
    //, dram_buffer1->address()
    //SetRuntimeArgs(program, void_data_kernel_noc1, core, {dram_buffer1->address()});
#endif
    if (timed)
    ...
    vector<uint32_t> result(single_tile_size);
    EnqueueReadBuffer(cq, dram_buffer2, result, true);
...
//#ifdef PRINT_DEBUG
    ...
    bfloat16* c_bf16 = reinterpret_cast<bfloat16*>(result.data());
    
    //int nn =  *(&c_bf16 + 1) - c_bf16;
    printf("Destination array size = %zu %zu %zu\n",result.size(), pattern_mat_val.size(), sparse_vec.size());

    for(uint32_t i= 0 ; i < 16384; i = i+1){
      printf("%f ", a_bf16[i].to_float());
      if(i % single_tile_size == 0){
        printf("\n");
      }
    }
    printf("\n");
    /*for(uint32_t i= 2040 ; i < 2048; i = i+1){
      printf("%f %f %f\n", a_bf16[i].to_float(), b_bf16[i].to_float(),c_bf16[i].to_float());
    }*/
    printf("results \n");

    for(uint32_t i= 0 ; i < result.size() * 2; i = i+1){
...
//#endif
    ...
    for (size_t j = 0; j < pattern_length; ++j){
      dense[j + pattern_length * (i % wrap)] = sparse[pattern[j] + delta * i];
      if(i == (count - 1)){ //To validate -l 2048 
      printf("%zu %zu %f\n", j + pattern_length * (i % wrap), pattern[j] + delta * i, dense[j + pattern_length * (i % wrap)]);
      }
    }
    }
    ...
    */
    ...
    std::vector<uint32_t> sparse_vec(sparse.size());
    //std::vector<uint32_t> pattern_mat_val(pattern_length);
    std::vector<uint32_t> pattern_mat_val(1024);
    ...
    uint32_t stride_len = pattern[1];
    for(uint32_t i=0 ; i < 1024 ; i=i+stride_len){
      pattern_mat_val[i] = 1;
    }
    EnqueueWriteBuffer(cq, dram_buffer, sparse_vec, false);
    EnqueueWriteBuffer(cq, dram_buffer1, pattern_mat_val, false);
...
//Host Code
  ...
  for (size_t i = 0; i < count; ++i){
    for (size_t j = 0; j < pattern_length; ++j){
      dense[j + pattern_length * (i % wrap)] = sparse[pattern[j] + delta * i];
      //if(i == 127 || i == 255 || i == 383){ //To validate -l 2048 
      //printf("%zu %zu %f\n", j + pattern_length * (i % wrap), pattern[j] + delta * i, dense[j + pattern_length * (i % wrap)]);
      //}
    }
  }
